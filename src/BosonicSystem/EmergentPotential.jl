...
for index in eachindex(quant_system.GLOB_EIG_E)
    ...
    ax.plot(T, real.(vcomp), label= L"Fed([V_{class} * V_{emerg}]) / \lambda")
    ax.set_xlabel(L"t")
    ax.set_ylabel(L"Feidelity")
    ax.set_title("CoupledHarmonicOscillator \n E = $ene, S = $entan")
    ax.legend()
    ax.grid(true)
    fig.savefig("data/V_emerg/emergentPotential_Index_18.png",)
    close(fig)
end