...
export BosonQuantumSystem, UpdateIndex, φ_λ
using QuantumOptics
abstract type QuantumSystem end
mutable struct BosonQuantumSystem <: QuantumSystem
    Hs::Operator
    Hc::Operator
    H::Operator
    V::Operator
    ...
    Ψ::Ket
    EΨ::Float64
    function BosonQuantumSystem(Hs::Operator, Hc::Operator, V::Operator)
        println("Initializing BosonQuantumSystem")
        system = new()
        system.Hs = Hs
        system.Hc = Hc
        system.V = V
...
function UpdateIndex(System::BosonQuantumSystem, index::Int64)
    System.index = index
...
=#
function φ_λ(t::Float64, system::BosonQuantumSystem)
